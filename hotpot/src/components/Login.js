...
const Login = () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      localStorage.setItem("restaurantOwnerEmail", formData.email);
      ...
      if (role === "ADMIN") {
        ...
        navigate("/restaurant/dashboard", { state: { email: formData.email } });

      } else {
        navigate("/select-category", { state: { email: formData.email } });
      }
    } else {
...
// };
  ...
  <nav className="navbar navbar-dark bg-dark px-3 py-2">
    ...
    <button className="btn btn-outline-light" onClick={() => navigate('/about')}>ABOUT US</button>
    <button className="btn btn-warning fw-bold" onClick={() => navigate('/register')}>REGISTER</button>
  </div>
</nav>
